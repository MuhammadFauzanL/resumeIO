...
const CertificationForm = () => {
    ...
    const handleRemove = (id) => updateResumeData({ certifications: certifications.filter(c => c.id !== id) });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {certifications.length === 0 && <p className="text-gray-500 text-sm text-center py-4">No certifications added.</p>}
                        ...
                        {certifications.map((cert) => (
                            <SortableItem key={cert.id} id={cert.id} onToggleExpand={() => toggleExpand(cert.id)}>
                                <div className="flex justify-between items-center w-full">
                                    <div className="font-medium text-gray-700">{cert.name || '(Certification Name)'}</div>
                                    <div className="flex items-center space-x-2">
                                        ...
                                        {expandedIds.includes(cert.id) ? <ChevronUp className="w-4 h-4 text-gray-500" /> : <ChevronDown className="w-4 h-4 text-gray-500" />}
                                    </div>
                                </div>
                                ...
                                {expandedIds.includes(cert.id) && (
                                    <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 gap-4 animate-fadeIn cursor-default" onClick={(e) => e.stopPropagation()}>
                                        ...
                                        <Input label="Date" placeholder="MM/YYYY" value={cert.date} onChange={(e) => handleChange(cert.id, 'date', e.target.value)} />
