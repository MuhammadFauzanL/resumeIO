...
const ReferenceForm = () => {
    ...
    const handleAdd = () => {
        const newRef = { id: generateId(), name: '', company: '', phone: '', email: '' }; // Added email
        updateResumeData({ references: [newRef, ...references] });
        setExpandedIds([newRef.id, ...expandedIds]);
    };

    const handleRemove = (id) => updateResumeData({ references: references.filter(r => r.id !== id) });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {references.length === 0 && <p className="text-gray-500 text-sm text-center py-4">No references added.</p>}
